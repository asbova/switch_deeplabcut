...
for i_animal = 1:size(uniqueSbj,2)
    ...
    for i_day = 1:numOfDays
        ...
        rpInRew = mpcParsed(lineIdx).Z'; % responding in reward port 
...
%         fzOut = mpcParsed(lineIdx).Y';
        ...
        for j = 1:trialNum
            ...
            if j < trialNum
                nextTS = trialStart(j+1); % get next trial's start time
            else
                nextTS = mpcParsed.X; % use end of recording as next trial start if this is the last trial of session
            end
            ...
            %trial(j).rewardRspTimeTrial = rpInRew(rpInRew>curTS & rpInRew<=curTE) - curTS;
            trial(j).rewardRspTimeTrial = rpInRew(rpInRew>curTS & rpInRew<=nextTS) - curTS; % changed this so that you see reward entries after reward delivery
